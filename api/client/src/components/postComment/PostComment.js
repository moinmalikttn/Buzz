...
const PostComment = ({ post, callBack, flag }) => {
  ...
  let FetchComments = async () => {
    axios
      .get(`${apiUrl}/postupload/comment/${post._id}`)
      .then((res) => {
  ...
  let addComment = async (e) => {
    ...
    await axios
      .post(`${apiUrl}/postupload/comment/${post._id}`, {
        id: post._id,
        comments: [
          {
  ...
  /*let editComment = (value)=>{
      ...
      axios.put(`${apiUrl}/postupload/comment/${post._id}/${value[1]._id}`,{
        comment:comment
      })
  ...
  let deleteComment = async (value) => {
    ...
    await axios
      .delete(`${apiUrl}/postupload/comment/${post._id}/${value[1]._id}`)
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
      ...
      {flag && (
        <div className="Postcomment">
          ...
          <div className="ReadComment">
            {readComment.map((value) => (
              <div className="showComment">
                <div className="UserContainer">
                  <div className="Row">
                    <h3>{value[1].name}</h3>
                  </div>
                  <div className="Row">{value[1].comment}</div>
                </div>
