...
function Topbar() {
  ...
  let getAllUsers = async () => {
    await axios
      ...
      .then((response) => {
        ...
        response.data.map((valueName) =>
          setAllusers((values) => [...values, valueName])
        );
      })
      .catch((err) => {
        console.log(err);
  ...
  let searchUser = () => {
    axios
      .get(`${apiUrl}/authusers/${user.profileObj.name}`)
      .then((value) => {
  ...
  let searchUserbyEmail = (list) => {
    ...
    list.map((user) => {
      console.log(user);
      return axios
        .get(`${apiUrl}/authusers/${user.Email}/Email`)
        .then((value) => {
          // console.log(value);
  ...
  let reqAccepted = async (value) => {
    await axios.delete(`${apiUrl}/friendrequest/${id}/request/${value.email}`);

    await axios.delete(
      `${apiUrl}/friendrequest/${value.id}/sentRequest/${user.profileObj.email}`
    );

    await axios.put(`${apiUrl}/users/${id}/follow`, {
      userId: value.id,
    });

    getFriendList();
    acceptedRequest({
      recieverEmail: user.profileObj.email,
