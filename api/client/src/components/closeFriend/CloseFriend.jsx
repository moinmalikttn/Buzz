  ...
  let callMe = async () => {
    ...
    try {
      if (currentUser.followings.length > 0 && checkIt()) setFollowed(1);
        ...
        axios
          .get(
            `${apiUrl}/friendrequest/${currentUser._id}/${user.email}/isrequested`
          )
          .then((value) => {
  ...
  const handleClick = async () => {
    ...
    try {
      if (followed === 1) {
        ...
        await axios.post(`${apiUrl}/friendrequest/${currentUser._id}/sent`, {
          Email: user.email,
        });
        sendRequest({
  ...
  return (
    <li className="sidebarFriend">
      ...
      <span className="sidebarFriendName">
        <Link to={`/feeds/userprofile/${user.name}`} id="UserNames">
          {user.name}
        </Link>
      </span>
